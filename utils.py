"""Shared losses, EMA/ramp-up helpers (training) and inference utilities."""
...
def load_model(model, weights_path, device):
    """Load a checkpoint (dict with a `model_state_dict` key, as saved by
    the training loop, or a bare state_dict) and return the model on
    `device` in eval mode."""
